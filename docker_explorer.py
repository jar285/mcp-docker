...
def generate_dockerfile(
    app_description: str = Field(description="Description of the application to containerize (e.g., 'Python Flask web app with Redis')"),
    app_type: str = Field(default="web", description="Application type (web, api, database, worker, static)"),
    base_image: str = Field(default="", description="Optional base image to use (e.g., 'python:3.9-alpine')"),
    include_comments: bool = Field(default=True, description="Whether to include explanatory comments in the Dockerfile")
) -> str:
    """Generate a Dockerfile based on application requirements described in natural language"""
    try:
        # Parse the application description to determine the appropriate base image and setup
        app_description = app_description.lower()
        
        # Determine language/framework from the description if base_image not specified
        detected_language = ""
        detected_version = ""
        detected_framework = ""
        detected_database = ""
        ...
        if "python" in app_description:
            detected_language = "python"
            if "3.9" in app_description:
                detected_version = "3.9"
            elif "3.10" in app_description:
                detected_version = "3.10"
            elif "3.11" in app_description:
                detected_version = "3.11"
            else:
                detected_version = "3.11"  # Default to latest stable
        elif "node" in app_description or "javascript" in app_description or "js" in app_description:
            detected_language = "node"
            if "18" in app_description:
                detected_version = "18"
            elif "16" in app_description:
                detected_version = "16"
            else:
                detected_version = "18"  # Default to latest LTS
        elif "java" in app_description:
            detected_language = "java"
            if "17" in app_description:
                detected_version = "17"
            elif "11" in app_description:
                detected_version = "11"
            else:
                detected_version = "17"  # Default to latest LTS
        elif "go" in app_description or "golang" in app_description:
            detected_language = "golang"
            detected_version = "1.20"  # Default to latest stable
        
        # Check for frameworks
        if "flask" in app_description:
            detected_framework = "flask"
        elif "django" in app_description:
            detected_framework = "django"
        elif "fastapi" in app_description:
            detected_framework = "fastapi"
        elif "express" in app_description:
            detected_framework = "express"
        elif "react" in app_description:
            detected_framework = "react"
        
        # Check for databases
        if "postgres" in app_description or "postgresql" in app_description:
            detected_database = "postgres"
        elif "mysql" in app_description:
            detected_database = "mysql"
        elif "mongodb" in app_description or "mongo" in app_description:
            detected_database = "mongodb"
        
        # Check for caching
        if "redis" in app_description:
            detected_cache = "redis"
        elif "memcached" in app_description:
            detected_cache = "memcached"
        
        # Use the specified base image or determine from detected language
        if base_image:
            dockerfile_base = base_image
        else:
            if detected_language == "python":
                if "alpine" in app_description:
                    dockerfile_base = f"python:{detected_version}-alpine"
                else:
                    dockerfile_base = f"python:{detected_version}-slim"
            elif detected_language == "node":
                if "alpine" in app_description:
                    dockerfile_base = f"node:{detected_version}-alpine"
                else:
                    dockerfile_base = f"node:{detected_version}-slim"
            elif detected_language == "java":
                dockerfile_base = f"openjdk:{detected_version}-slim"
            elif detected_language == "golang":
                dockerfile_base = f"golang:{detected_version}-alpine"
            else:
                # Default to a minimal base image if language not detected
        ...
        # Add header comment
        if include_comments:
            dockerfile.append(f"# Dockerfile for {app_description}")
            dockerfile.append(f"# Generated by Docker Explorer MCP Server")
            dockerfile.append("")
        
        # Add base image
        dockerfile.append(f"FROM {dockerfile_base}")
        dockerfile.append("")
        
        # Add maintainer label
        if include_comments:
            dockerfile.append("# Set metadata")
        dockerfile.append("LABEL maintainer=\"Docker Explorer User <user@example.com>\"")
        dockerfile.append("")
        ...
        # Add environment variables based on detected components
        env_vars = []
        if detected_framework == "flask" or detected_framework == "fastapi":
            env_vars.append("PYTHONUNBUFFERED=1")
        elif detected_framework == "django":
            env_vars.append("PYTHONUNBUFFERED=1")
            env_vars.append("DJANGO_SETTINGS_MODULE=config.settings")
        elif detected_framework == "express" or detected_framework == "react":
        ...
        if env_vars:
            ...
            for env_var in env_vars:
                dockerfile.append(f"ENV {env_var}")
            dockerfile.append("")
        
        # Install dependencies based on detected language and framework
        if detected_language == "python":
            if include_comments:
                dockerfile.append("# Install Python dependencies")
            dockerfile.append("COPY requirements.txt .")
            dockerfile.append("RUN pip install --no-cache-dir -r requirements.txt")
            dockerfile.append("")
        elif detected_language == "node":
            if include_comments:
                dockerfile.append("# Install Node.js dependencies")
            dockerfile.append("COPY package*.json ./")
            dockerfile.append("RUN npm ci --only=production")
            dockerfile.append("")
        elif detected_language == "java":
            if include_comments:
                dockerfile.append("# Copy Maven/Gradle files")
            if "maven" in app_description:
                dockerfile.append("COPY pom.xml .")
                dockerfile.append("RUN mvn dependency:go-offline")
            elif "gradle" in app_description:
                dockerfile.append("COPY build.gradle settings.gradle ./")
                dockerfile.append("COPY gradle ./gradle")
                dockerfile.append("RUN ./gradlew dependencies")
            dockerfile.append("")
        
        # Copy application code
        if include_comments:
            dockerfile.append("# Copy application code")
        dockerfile.append("COPY . .")
        dockerfile.append("")
        
        # Add user for security (non-root)
        if include_comments:
            dockerfile.append("# Create a non-root user for security")
        dockerfile.append("RUN addgroup --system appgroup && adduser --system --ingroup appgroup appuser")
        dockerfile.append("USER appuser")
        dockerfile.append("")
        
        # Expose ports based on app type
        if app_type == "web" or app_type == "api":
            ...
            if include_comments:
                dockerfile.append(f"# Expose port {port} for the application")
        ...
        if app_type == "web" or app_type == "api":
            if include_comments:
                dockerfile.append("# Add healthcheck to verify the application is running")
            dockerfile.append("HEALTHCHECK --interval=30s --timeout=3s CMD curl -f http://localhost:${PORT:-8080}/health || exit 1")
            dockerfile.append("")
        
        # Add CMD to start the application
        if include_comments:
            dockerfile.append("# Command to run the application")
        
        if detected_language == "python":
            if detected_framework == "flask":
                dockerfile.append("CMD [\"python\", \"app.py\"]")
            elif detected_framework == "fastapi":
                dockerfile.append("CMD [\"uvicorn\", \"main:app\", \"--host\", \"0.0.0.0\", \"--port\", \"8000\"]")
            elif detected_framework == "django":
                dockerfile.append("CMD [\"python\", \"manage.py\", \"runserver\", \"0.0.0.0:8000\"]")
            else:
                dockerfile.append("CMD [\"python\", \"main.py\"]")
        elif detected_language == "node":
            if detected_framework == "express":
                dockerfile.append("CMD [\"node\", \"server.js\"]")
            else:
                dockerfile.append("CMD [\"npm\", \"start\"]")
        elif detected_language == "java":
            if "spring" in app_description:
                dockerfile.append("CMD [\"java\", \"-jar\", \"/app/target/app.jar\"]")
            else:
                dockerfile.append("CMD [\"java\", \"-jar\", \"/app/target/app.jar\"]")
        elif detected_language == "golang":
            dockerfile.append("CMD [\"./app\"]")
        else:
            dockerfile.append("CMD [\"sh\", \"-c\", \"echo 'Application starting...' && exec /app/start.sh\"]")
        
        # Add additional notes and recommendations
        if include_comments:
            dockerfile.append("")
            dockerfile.append("# Notes:")
            dockerfile.append("# - This Dockerfile is a starting point and may need adjustments for your specific application")
            if detected_database:
                dockerfile.append(f"# - For {detected_database} database connection, consider using environment variables for credentials")
            if detected_cache:
                dockerfile.append(f"# - For {detected_cache} caching, ensure your application is configured to connect to it")
            dockerfile.append("# - For production, consider using multi-stage builds to reduce image size")
        
        # Join all lines to create the final Dockerfile
        dockerfile_content = "\n".join(dockerfile)
        
        # Create the response
        result = f"# Dockerfile for {app_description}\n\n```dockerfile\n{dockerfile_content}\n```\n\n"
        
        # Add usage instructions
        result += "## Usage Instructions\n\n"
        result += "1. Save the above content to a file named `Dockerfile` in your project directory\n"
        result += "2. Build the Docker image:\n\n"
        result += "```bash\n"
        result += "docker build -t your-app-name .\n"
        result += "```\n\n"
        result += "3. Run the container:\n\n"
        result += "```bash\n"
        ...
        if app_type == "web" or app_type == "api":
            ...
            if detected_framework == "flask":
                ...
                port = "3000"
            result += f"docker run -p {port}:{port} your-app-name\n"
        else:
            result += "docker run your-app-name\n"
